...
def load_market_data(bench_ticker="^IXIC"):
    # 12 Activos: Los originales 10 + AGG + SHV
    assets = ["COST", "WMT", "NVDA", "AAPL", "IBTA", "NEE", "JPM", "V", "DUK", "VSAT", "AGG", "SHV"]
...
with st.sidebar:
    ...
    for asset in asset_list:
        # Usamos flotantes y permitimos decimales (step=0.01)
        val = st.slider(f"% {asset}", 0.0, 100.0, initial_weight, step=0.01)
...
with tab1:
    ...
    with col_b:
        ...
        fig_area = go.Figure()
        fig_area.add_trace(go.Scatter(x=port_cum.index, y=port_cum, mode='lines', name='Mi Cartera (12 Activos)', line=dict(shape='spline', smoothing=1, color=C_PURP, width=3), fill='tozeroy', fillcolor='rgba(139,92,246,0.15)'))
    ...
    st.markdown("<div class='glass-box'>", unsafe_allow_html=True)
    table_html = f"""
    <div class='sub-header'>Resumen Algorítmico Activo (12)</div>
