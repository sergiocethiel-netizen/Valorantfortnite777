...
def load_market_data(bench_ticker="^IXIC"):
    assets = ["COST", "WMT", "NVDA", "AAPL", "IBTA", "NEE", "JPM", "V", "DUK", "VSAT"]
...
with st.sidebar:
    ...
    for asset in asset_list:
        val = st.slider(f"% {asset}", 0, 100, 10)
        raw_weights.append(val)
        
    w_sum = sum(raw_weights)
    if w_sum == 0:
        raw_weights = [10]*len(asset_list)
        w_sum = 100
        
    w_arr = np.array([w/w_sum for w in raw_weights])
    
    if w_sum != 100:
        st.markdown(f"<div style='color:#ef4444; font-size:12px; margin-bottom:10px;'>⚠️ Sum {w_sum}%. Auto-normalizing to 1.0.</div>", unsafe_allow_html=True)
...
# TABS INTERACTIVOS
# ==========================================
tab1, tab2, tab3 = st.tabs(["📊 Performance Overview", "🌐 Markowitz & Risk", "🌀 Chaos Theory Core"])
...
with tab1:
    ...
    with col_a:
        ...
        fig_donut = go.Figure(go.Pie(labels=asset_list, values=w_arr*100, hole=0.75, marker_colors=[C_CYAN, C_PURP, C_PINK, "#3b82f6"], textinfo='none'))
    ...
    with col_b:
        ...
        fig_area = go.Figure()
        fig_area.add_trace(go.Scatter(x=port_cum.index, y=port_cum, mode='lines', name='Mi Cartera', line=dict(shape='spline', smoothing=1, color=C_PURP, width=3), fill='tozeroy', fillcolor='rgba(139,92,246,0.15)'))
    ...
    st.markdown("<div class='glass-box'>", unsafe_allow_html=True)
    table_html = f"""
    <div class='sub-header'>Resumen Algorítmico Activo</div>
    <table class="custom-table" style="margin-top:10px;">
        ...
        <tr><td>Expected Shortfall Promedio</td><td align="right" style="color:#ef4444;">{cvar_val:.2f}% Drowdown</td></tr>
    </table>
    """
